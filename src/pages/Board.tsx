...
export function BoardPage() {
  ...
  return (
    <AppShell title="排程看板" subtitle="订单池拖动 + 产能热力 + 可行性概览">
      {session && (
        <div className="mb-8 grid gap-6 xl:grid-cols-[320px_1fr]">
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 p-6 text-white shadow-[0_30px_70px_-20px_rgba(0,0,0,0.6)] ring-1 ring-slate-800/60">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(96,165,250,0.3),transparent_35%),radial-gradient(circle_at_70%_80%,rgba(16,185,129,0.2),transparent_40%)] opacity-70" />
            <div className="relative flex flex-col items-center gap-5 text-center">
              <div className="relative h-28 w-28 overflow-hidden rounded-3xl border border-white/20 bg-white/10 shadow-2xl shadow-emerald-500/20">
                {avatarUrl ? (
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-center gap-2 text-xs uppercase tracking-[0.35em] text-slate-200">
                  <BadgeCheck className="h-4 w-4 text-emerald-300" />
                  个人卡片
                </div>
                <h2 className="text-2xl font-semibold">{session.emp_name}</h2>
                <div className="flex flex-col items-center gap-2 text-sm text-slate-200">
                  ...
                  <span className="rounded-full bg-emerald-100/15 px-3 py-1 text-xs font-semibold text-emerald-100 ring-1 ring-emerald-200/40 shadow-sm">
                    角色：{session.role || '—'}
                  </span>
                </div>
              </div>
              {avatarError && (
                <div className="w-full rounded-xl bg-red-50/90 p-3 text-sm text-red-700 ring-1 ring-red-200">
                  {avatarError}
                </div>
              )}
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl bg-white/90 p-5 shadow-xl shadow-slate-200/40 ring-1 ring-slate-200 backdrop-blur">
              ...
              <div className="mt-4 space-y-3 text-sm">
                ...
                <p className="rounded-xl bg-slate-50/90 p-3 text-xs text-slate-500 ring-1 ring-slate-200">
                  接入 Supabase 订单/日历后将即时刷新排程预览。
                </p>
              </div>
            </div>
      ...
      <div className="mt-6 rounded-2xl bg-white/90 p-5 shadow-xl shadow-slate-200/40 ring-1 ring-slate-200 backdrop-blur">
